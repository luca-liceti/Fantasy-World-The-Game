...
| 1.9 | 2026-03-11 | **PERFORMANCE: Async Board Generation** (`hex_board.gd`, `main.gd`): Redesigned the board generation sequence to run concurrently with the deck selection UI. Implemented a coroutine-based `generate_board_async()` that yields to the engine between chunks, preventing main thread stalls. Deck selection now appears instantly after clicking Play, dramatically improving perceived load times and user experience. |
